...
const frontClass = computed(() => {
  const base = 'absolute z-30 w-full h-full origin-bottom transition-all duration-300 ease-in-out rounded-tl-[5px] rounded-tr-[10px] rounded-br-[10px] rounded-bl-[10px]'
  return open.value ? `${base} [transform:skew(15deg)_scaleY(0.6)]` : `${base} group-hover:[transform:skew(15deg)_scaleY(0.6)]`
})
...
<template>
  <div :style="{ transform: `scale(${props.size})` }" :class="props.class">
    <div :class="folderClass" @click="handleClick">
      <div
      ...
      >
        <span
          ...
          :style="{ backgroundColor: folderBackColor }"
        />
        <div v-for="(_item, i) in papers" :key="i" :class="getPaperClasses(i)" :style="getPaperStyle(i)">
          <slot :name="`item-${i + 1}`" />
        </div>
